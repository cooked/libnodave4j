...
//import sc.siemens.base.SlaveArea;
import sc.siemens.base.ReadFunctionGroup;
import sc.siemens.base.SlaveAreaOffset;
...
public class BatchRead<K> {
    private final List<KeyedS7Locator<K>> requestValues = new ArrayList<KeyedS7Locator<K>>();
    ...
    public List<KeyedS7Locator<K>> getRequestValues() {
    ...
    /*public void addLocator(K id, ProfiLocator locator) {
        addLocator(new KeyedProfiLocator<K>(id, locator));
    }

    public void addLocator(K id, int slaveId, int range, int offset, int dataType) {
        addLocator(new KeyedProfiLocator<K>(id, slaveId, range, offset, dataType));
    }

    public void addLocator(K id, int slaveId, int range, int offset, byte bit) {
        addLocator(new KeyedProfiLocator<K>(id, slaveId, range, offset, bit));
    }*/
    ...
    private void addLocator(KeyedS7Locator<K> locator) {
        requestValues.add(locator);
    }
    
    public void addLocator(K id, BaseLocator<?> locator) {
    	addLocator(new KeyedS7Locator<K>(id, locator));
    }
    ...
    private void doPartition() {
    	...
    	Map<SlaveAreaOffset, List<KeyedS7Locator<K>>> slaveAreaOffsetBatch = new HashMap<SlaveAreaOffset, List<KeyedS7Locator<K>>>();
    	//Map<Integer, List<KeyedS7Locator<K>>> slaveRangeBatch = new HashMap<Integer , List<KeyedS7Locator<K>>>();

        // Separate the batch into slave ids and read functions.
        List<KeyedS7Locator<K>> functionList;
        for (KeyedS7Locator<K> locator : requestValues) {
            // Find the function list for this slave and area. Create it if necessary.
            functionList = slaveAreaOffsetBatch.get(locator.getSlaveAreaOffset());
            if (functionList == null) {
                functionList = new ArrayList<KeyedS7Locator<K>>();
                slaveAreaOffsetBatch.put(locator.getSlaveAreaOffset(), functionList);
            }
        ...
        // parts as necessary.
        Collection<List<KeyedS7Locator<K>>> functionLocatorLists = slaveAreaOffsetBatch.values();
        FunctionLocatorComparator comparator = new FunctionLocatorComparator();
        functionGroups = new ArrayList<ReadFunctionGroup<K>>();
        for (List<KeyedS7Locator<K>> functionLocatorList : functionLocatorLists) {
            ...
            if (contiguousRequests)
                ...
                // this function
                int maxReadCount = MemoryArea.getMaxReadCount(functionLocatorList.get(0).getArea());
     ...
     */
    private void createRequestGroups(List<ReadFunctionGroup<K>> functionGroups, List<KeyedS7Locator<K>> locators,
            int maxCount) {
        ReadFunctionGroup<K> functionGroup;
        KeyedS7Locator<K> locator;
        int index;
        int endOffset;
    ...
    private void createContiguousRequestGroups(List<ReadFunctionGroup<K>> functionGroups,
            List<KeyedS7Locator<K>> locators) {
    ...
    class FunctionLocatorComparator implements Comparator<KeyedS7Locator<K>> {
        public int compare(KeyedS7Locator<K> ml1, KeyedS7Locator<K> ml2) {
