...
import sc.siemens.code.ExceptionCode;
import sc.siemens.exception.S7TransportException;

abstract public class S7Response extends S7Message {
    protected static final byte MAX_FUNCTION_CODE = (byte) 0x80;

    /*public static S7Response createModbusResponse(ByteQueue queue) throws S7TransportException {
        int slaveId = S7Utils.popUnsignedByte(queue);
        ...
        if (functionCode == FunctionCode.READ_COILS)
            ...
            response = new ReadDBAreaResponse(slaveId);
    ...
    S7Response(int slaveId) throws S7TransportException {
        super(slaveId);
    }
    ...
    final protected void writeImpl(ByteQueue queue) {
        if (isException()) {
            queue.push((byte) (getArea() + MAX_FUNCTION_CODE));
...
*/
    ...
    /*public static void main(String[] args) throws Exception {
        ...
        S7Response r = createModbusResponse(queue);
