...
abstract public class S7Request extends S7Message {
	
    /*public static ProfiRequest createProfiRequest(ByteQueue queue) throws ProfiTransportException {
        
    	int slaveId = ProfiUtils.popUnsignedByte(queue);
        byte functionCode = queue.pop();

        ProfiRequest request = null;
        if (functionCode == FunctionCode.READ_COILS)
            ...
            request = new ReadDBAreaRequest(slaveId);
    ...
    S7Request(int slaveId) throws S7TransportException {
        super(slaveId);
    }

    /*public S7Response handle(ProcessImage processImage) throws S7TransportException {
        try {
            try {
    ...
    //abstract S7Response handleImpl(ProcessImage processImage) throws S7TransportException;

    //abstract protected void readRequest(ByteQueue queue);
    //abstract protected void writeRequest(ByteQueue queue);
    
    S7Response handleException(byte exceptionCode) throws S7TransportException {
        S7Response response = getResponseInstance(slaveId);
        response.setException(exceptionCode);
        return response;
    }

    abstract S7Response getResponseInstance(int slaveId) throws S7TransportException;

    /*@Override
    final protected void writeImpl(ByteQueue queue) {
        //queue.push(getArea());
        //queue.push(getAreaNr());
        //writeRequest(queue);
    }*/
