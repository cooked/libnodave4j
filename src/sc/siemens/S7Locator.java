...
public class S7Locator {
    ...
    /*public S7Locator(SlaveAndArea slaveAndArea, int offset, int dataType) {
        this(slaveAndArea, offset, dataType, (byte) -1);
    }

    public S7Locator(int slaveId, int area, int offset, int dataType) {
        this(new SlaveAndArea(slaveId, area), offset, dataType, (byte) -1);
    }

    public S7Locator(int slaveId, int area, int offset, byte bit) {
        this(new SlaveAndArea(slaveId, area), offset, DataType.BOOL, bit);
        if (area != MemoryArea.DB && area != MemoryArea.DI)
            throw new S7IdException("Bit requests can only be made from holding registers and input registers");
    }

    public S7Locator(int slaveId, int range, int offset, int dataType, byte bit) {
        this(new SlaveAndArea(slaveId, range), offset, dataType, bit);
    }

    private S7Locator(SlaveAndArea slaveAndArea, int offset, int dataType, byte bit) {
        this.slaveAndArea 	= slaveAndArea;
        this.offset 		= offset;
    ...
    public SlaveAndArea getSlaveAndArea() {
        return slaveAndArea;
    }

    public int getEndOffset() {
        return offset + DataType.getByteCount(dataType) - 1;
    }
    ...
    /*public Object bytesToValue(byte[] data, int requestOffset) {
        ...
        return bytesToValue(data, slaveAndArea.getArea(), offset - requestOffset, dataType, bit);
    }
    ...
    /*public static void main(String[] args) {
        S7Locator ml = new S7Locator(0, 0, 0, DataType.FOUR_BYTE_BCD);
        int i = 12345678;
